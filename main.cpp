...
int main()
{
    ...
    auto add =    std::make_unique<BinArithOpNode>(bin_arith_op_type::add,
                                                std::move(lit1),
                                                std::move(lit2));
    auto assign_init = std::make_unique<AssignNode>(std::move(var_a1), std::move(add));
    ...
    auto cond =   std::make_unique<BinLogicOpNode>(bin_logic_op_type::less,
                                                 std::move(var_a3),
                                                 std::move(lit10));

    auto var_a4 =   std::make_unique<VarNode>("a");
    auto lit1_inc = std::make_unique<ValueNode>(1);
    auto add_inc =  std::make_unique<BinArithOpNode>(bin_arith_op_type::add,
                                                    std::move(var_a4),
                                                    std::move(lit1_inc));
    
    auto assign_inc = std::make_unique<AssignNode>(std::make_unique<VarNode>("a"), std::move(add_inc));

    auto while_body = std::make_unique<ScopeNode>();
    while_body->add_statement(std::move(assign_inc));

    auto while_node = std::make_unique<WhileNode>(std::move(cond), std::move(while_body));
