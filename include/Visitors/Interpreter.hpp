...
public:
    ...
    void visit(BinArithOpNode& node) override
    {
        ...
        switch (node.op()) {
            ...
            case bin_arith_op_type::div:
                if (right_res == 0) {
                    throw std::runtime_error(make_runtime_error(
                        node.location(), "Division by zero"));
                }
    ...
    void visit(UnOpNode& node) override
    {
        ...
        if (!operand) {
            throw std::runtime_error(make_runtime_error(
                node.location(), "UnOpNode missing operand"));
        }
        operand->accept(*this);
        switch (node.op()) {
    ...
    void visit(AssignNode& node) override
    {
        ...
        if (lhs == nullptr) {
            throw std::runtime_error(make_runtime_error(
                node.location(), "AssignNode's lhs is nullptr"));
        }
        bool is_var = lhs->node_type() == base_node_type::var;
        if (!is_var) {
            throw std::runtime_error(make_runtime_error(
                node.location(), "AssignNode lhs must be var"));
        }
        ...
        if (!operand) {
            throw std::runtime_error(make_runtime_error(
                node.location(), "AssignNode missing operand"));
        }
