...
void AddPrintedBinaryExpr(ast::ScopeNode& scope,
                          ast::bin_arith_op_type op,
                          int64_t lhs,
                          int64_t rhs)
{
    scope.add_statement(
        std::make_unique<ast::PrintNode>(std::make_unique<ast::BinArithOpNode>(
            op,
...
{
    scope.add_statement(
        std::make_unique<ast::PrintNode>(std::make_unique<ast::BinLogicOpNode>(
            op,
            std::make_unique<ast::ValueNode>(lhs),
            std::make_unique<ast::ValueNode>(rhs))));
}
...
TEST(InterpreterExprTest, ValueNodeTest)
{
    ...
    EXPECT_EQ(RunValueNodePrintCase(std::numeric_limits<int64_t>::max()),
              "9223372036854775807\n");
    EXPECT_EQ(RunValueNodePrintCase(std::numeric_limits<int64_t>::min()),
              "-9223372036854775808\n");
}
...
TEST(InterpreterExprTest, VarNodeTest)
{
    ...
    root.add_statement(std::make_unique<ast::VarDeclNode>(
        "x", std::make_unique<ast::ValueNode>(42)));
    root.add_statement(
...
TEST(InterpreterExprTest, UnOpNodeTest)
{
    ast::ScopeNode root;
    root.add_statement(
        std::make_unique<ast::PrintNode>(std::make_unique<ast::UnOpNode>(
            ast::unop_node_type::neg, std::make_unique<ast::ValueNode>(5))));
    root.add_statement(
        std::make_unique<ast::PrintNode>(std::make_unique<ast::UnOpNode>(
            ast::unop_node_type::pos, std::make_unique<ast::ValueNode>(7))));
    root.add_statement(std::make_unique<ast::PrintNode>(
        std::make_unique<ast::UnOpNode>(ast::unop_node_type::logical_not,
...
{
    ast::PrintNode valid_node(
        std::make_unique<ast::ExprNode>(std::make_unique<ast::ValueNode>(123)));
