...
TEST(ParserTest, WhileLoop)
{
    ...
    auto& ast = driver.get_ast();
    const auto& stmts =
        static_cast<const ast::ScopeNode*>(ast.root())->statements();
    ...
    EXPECT_EQ(while_node->condition()->node_type(),
              ast::base_node_type::bin_logic_op);
...
TEST(ParserTest, EmptyInput)
{
    ...
    auto& ast = driver.get_ast();
    EXPECT_EQ(
        static_cast<const ast::ScopeNode*>(ast.root())->statements().size(), 0);
}
